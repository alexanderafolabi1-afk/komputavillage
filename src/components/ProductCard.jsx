...
import { getWhatsAppLink, formatPrice } from '../data/products'

const stockConfig = {
  'in-stock': { label: 'In Stock', className: 'text-electric bg-electric/10' },
  'low-stock': { label: 'Low Stock', className: 'text-village bg-village/10' },
  'sold-out': { label: 'Sold Out', className: 'text-red-400 bg-red-400/10' },
}
...
export default function ProductCard({ product }) {
  ...
  return (
    <article className="group bg-dark-card border border-dark-border rounded-2xl overflow-hidden hover:border-electric/30 transition-all duration-300 flex flex-col">
      {/* Image */}
      ...
      <div className="p-4 flex flex-col flex-grow gap-3">
        ...
        {isSoldOut ? (
          <button
            disabled
            className="w-full py-2.5 rounded-xl bg-dark-border text-gray-500 text-sm font-dm font-medium cursor-not-allowed"
          >
            Sold Out
          </button>
        ) : (
          <a
            ...
            className="w-full py-2.5 rounded-xl bg-[#25D366] hover:bg-[#20bd5a] text-white text-sm font-dm font-semibold flex items-center justify-center gap-2 transition-colors active:scale-[0.98]"
          >
            <WhatsAppIcon />
            Order via WhatsApp
          </a>
